        ...
        for rule in not_processed_rules:
            ...
            if rule["enabled"]:
                ...
                pending_vote = {"authorperm": authorperm, "voter": rule["account"], "vote_weight": vote_weight, "comment_timestamp": post["created"].replace(tzinfo=None),
                                "vote_delay_min": rule["minimum_vote_delay_min"], "created": datetime.utcnow(), "min_vp": rule["min_vp"], "vote_when_vp_reached": False,
                                "vp_reached_order": 1, "max_net_votes": rule["max_net_votes"], "max_pending_payout": rule["max_pending_payout"],
